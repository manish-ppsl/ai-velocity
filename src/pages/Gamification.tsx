...
const user = mockLeaderboard[0]; // Sarah Chen as the profile example
      ...
      <div className="p-6 lg:p-8 space-y-6">
        ...
        >
          <div className="flex flex-col md:flex-row items-center gap-6">
            <div className="w-20 h-20 rounded-2xl bg-primary/20 flex items-center justify-center text-2xl font-bold text-primary animate-float">
              {user.avatar}
            </div>
            <div className="text-center md:text-left flex-1">
              ...
              <div className="flex items-center gap-3 mt-2 justify-center md:justify-start">
                <span className="px-3 py-1 rounded-full bg-glow-rose/15 text-glow-rose text-xs font-mono">
                  L{user.level} — {user.levelName}
                </span>
                <span className="text-sm text-glow-warning flex items-center gap-1">
                  <Flame className="w-4 h-4" /> {user.streak} week streak
                </span>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-6 text-center">
              <div>
                <div className="font-mono text-2xl font-bold gradient-text">{user.aiContribution}%</div>
                <div className="text-[10px] text-muted-foreground">AI Contribution</div>
              </div>
              <div>
                <div className="font-mono text-2xl font-bold text-foreground">{user.productivityIndex}</div>
                <div className="text-[10px] text-muted-foreground">Productivity</div>
              </div>
              <div>
                <div className="font-mono text-2xl font-bold text-foreground">{user.qualityScore}%</div>
        ...
        >
          <h3 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
            <Zap className="w-4 h-4 text-primary" /> Level Progression
          </h3>
          <div className="flex items-center gap-2">
            {mockLevels.map((lvl, i) => {
              ...
              return (
                <div key={lvl.level} className="flex-1 flex flex-col items-center gap-2">
                  <div className={cn(
                    "w-10 h-10 rounded-full flex items-center justify-center font-mono text-xs font-bold transition-all",
          ...
          <div className="mt-4 h-2 rounded-full bg-secondary overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${(user.level / 6) * 100}%` }}
        ...
        >
          <h3 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
            <Award className="w-4 h-4 text-glow-warning" /> Badges & Achievements
          </h3>
            ...
            {mockBadges.map((badge, i) => (
              <motion.div
                ...
                className={cn(
                  ...
                  badge.earned
                    ? "glass-card-glow hover:scale-105 cursor-pointer"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            <h3 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
              <Flame className="w-4 h-4 text-glow-warning" /> Active Streaks
            </h3>
            <div className="space-y-3">
              {[
                { name: "Weekly AI PR", current: 14, best: 14, active: true },
                { name: "Merge without rewrite", current: 8, best: 12, active: true },
                { name: "High-quality AI", current: 6, best: 9, active: true },
              ].map((streak) => (
                <div key={streak.name} className="flex items-center justify-between p-3 rounded-lg bg-secondary/30">
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground">Best: {streak.best} weeks</div>
                  </div>
          ...
          >
            ...
            <div className="space-y-3">
              {[
                { name: "AI Adoption Sprint", type: "Entity vs Entity", progress: 72, end: "5 days left" },
